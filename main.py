...
def generate_risk_vector_format(population_filepath: str):
    ...
    map_maker = risk_map_maker(population_filepath=population_filepath,
                               start_latlng=dallas_to_wichita_start_latlng,
                               dest_latlng=dallas_to_wichita_dest_latlng)
    start_whole = perf_counter()
    ...
    finish_whole = perf_counter()
    print(f'whole process took: {finish_whole - start_whole}')


if __name__ == '__main__':
    filepath = getcwd() + '\\Kontur_Population_US_20231029.gpkg'  # filepath to the data source
    generate_risk_vector_format(population_filepath=filepath)  # generate risk map in vector format
