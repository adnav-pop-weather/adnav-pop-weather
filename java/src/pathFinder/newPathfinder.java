...
public class newPathfinder {
    ...
    private static newPathfinder instance;

    private Graph riskGraph;
    private PopulationGraphGenerator graphGenerator;

    public static newPathfinder getPathfinderInstance() {
        if (instance == null) {
            instance = new newPathfinder();
        }
        return instance;
    }

    public void runScript(double startLat, double startLng, double destLat, double destLng) {
        ...
        String[] python_script_cmd = {"cmd.exe", "/C", "\"set", "PATH=" + existingPath, "&&",
                "conda", "run", "--name", "adnav", "python",
                "C:\\Users\\assist-lab\\Desktop\\adnav-pop-weather\\java\\src\\pathFinder\\main.py",
        ...
        try {
            ...
            Thread riskMapThread = new Thread(() -> {
                try {
                    int exitCode = process.waitFor(); // python script exit code.
                    if (exitCode == 0) {
                        // python script terminated gracefully. Offline_risk.gpkg exists, now build graph.
                        buildGraph();
                    } else {
    ...
    public void buildGraph() throws IOException {
        File directory = new File("C:\\Users\\assist-lab\\Desktop\\adnav-pop-weather\\java\\src\\pathFinder\\data");
        ...
        String myDataset = "offline_risk.gpkg";
        DataStore store = null;
        File in_file = new File("C:\\Users\\assist-lab\\Desktop\\adnav-pop-weather\\java\\src\\pathFinder\\data\\" + myDataset);
        ...
        assert store != null; // accept that the store isn't null so we don't get a nullptr exception.
        String[] typeNames = store.getTypeNames();
        String risk_feature_name = typeNames[0];
        // Access to geopackage. geopackage is the source for the features.
        SimpleFeatureSource offlineRiskFeatureSource = store.getFeatureSource(risk_feature_name);

        SimpleFeatureCollection griddedFeatures = offlineRiskFeatureSource.getFeatures();
        ...
        graphGenerator = new PopulationGraphGenerator(new BasicGraphBuilder(), relationships);

        griddedFeatures.accepts(
                new FeatureVisitor() {
        ...
        riskGraph = graphGenerator.getGraph(); // wallah the graph
    }

    public ArrayList<ArrayList<graph1.Node>> getPath(double [] currentPosition, double [] destination) throws Exception {
        // make geometry factory to make points which we then query the graph for the features we start at.
        GeometryFactory gf = new GeometryFactory(new PrecisionModel(), 4326);

        double currentPosLat = currentPosition[0];
        double currentPosLng = currentPosition[1];
        Point startPoint = gf.createPoint(new Coordinate(currentPosLng, currentPosLat)); // TODO possible incorrect order of coordinates.

        Node startNode = (Node) graphGenerator.getNode(startPoint.buffer(0.00001));


        double destPosLat = destination[0];
        double destPosLng = destination[1];
        Point endPoint = gf.createPoint(new Coordinate(destPosLng, destPosLat));
        Node destNode = (Node) graphGenerator.getNode(endPoint.buffer(0.00001));
        ...
        AStarIterator.AStarFunctions pathfindingFunctions = new AStarIterator.AStarFunctions(destNode) {
            ...
            public double cost(AStarIterator.AStarNode aStarNode, AStarIterator.AStarNode aStarNode1) {
                ...
                double risk = (double) aSN1Feature.getAttribute("offline_risk");
            ...
            public double h(Node node) {
                ...
                Polygon currentNodePoly = ((Polygon)((SimpleFeature) node.getObject()).getDefaultGeometry());
                Polygon destinationPoly = ((Polygon)((SimpleFeature) node.getObject()).getDefaultGeometry());
                double heuristic_distance = currentNodePoly.getCentroid().distance(destinationPoly.getCentroid());
                double currentNodeRisk = (double) ((SimpleFeature) node.getObject()).getAttribute("offline_risk");
                return heuristic_distance * currentNodeRisk;
            }
        };
        AStarIterator aStarIterator = new AStarIterator(startNode, pathfindingFunctions);
        // make the astar pathfinder
        AStarShortestPathFinder pathfinder = new AStarShortestPathFinder(riskGraph, startNode, destNode, pathfindingFunctions);
        ...
        // Now we have the path, and need to convert to the proper format from the og team.
        ArrayList<ArrayList<graph1.Node>> newWaypoints = new ArrayList<>();
        for (Node node : dronePathNodes) {
            Coordinate centerCoord = ((Polygon) ((SimpleFeature) node.getObject()).getDefaultGeometry()).getCentroid().getCoordinate();
            System.out.println(centerCoord);
            double[] coordinates = {centerCoord.y, centerCoord.x}; // TODO Inspect this as a point of error.
            ArrayList<graph1.Node> nodeList = new ArrayList<>();
            nodeList.add(new graph1.Node(coordinates)); // replicating the first team's code here.
            newWaypoints.add(
                   nodeList
            );
        }
