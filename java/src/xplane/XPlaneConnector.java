...
public class XPlaneConnector
{
    ...
    public static float [] aircraftElevation;
...
//            xpc.sendDREF("sim/cockpit/autopilot/alt_hold_ft_fms",altitude);
    ...
    public void setTargetElevation(int elevation){
        try {
            xpc.sendDREF("sim/cockpit2/gauges/indicators/radio_altimeter_height_ft_pilot", elevation);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void setDesRate(int desRate){
        try {
            xpc.sendDREF("sim/cockpit2/gauges/indicators/vvi_fpm_copilot", desRate);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//            aircraftVertSpeed = xpc.getDREF("sim/cockpit2/autopilot/des_adjust");
//
//            System.out.println("Vertical Speed: "+aircraftVertSpeed[0]);

            aircraftElevation = xpc.getDREF("sim/cockpit2/gauges/indicators/radio_altimeter_height_ft_pilot");
            fuelQuantity = xpc.getDREF("sim/flightmodel/weight/m_fuel");
            wheelBrakes = xpc.getDREF("sim/flightmodel/controls/parkbrake");
            reverse = xpc.getDREF("sim/cockpit/warnings/annunciators/reverse");

            airBrakesON = controlSurfaces[0][0] == 1.0f;
            wheelBrakesON = wheelBrakes[0] == 1.0f;
            reverseON = reverse[0] != 0.0f;
...
//        }
        ...
        return new FlightData((int) values [6][0], (int) values[5][0], (int) values[0][0], (int) values[1][0], values[2][0], values[3][0], values[4][0], enginesOK, wheelBrakesON, airBrakesON, reverseON);
    }
}
