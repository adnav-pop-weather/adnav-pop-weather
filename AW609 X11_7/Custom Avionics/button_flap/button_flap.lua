...
defineProperty("cs10",globalPropertyi("sim/cockpit2/switches/custom_slider_on[10]"))--entry stairs
...
components = {
    ...
    button {
        ...
        onMouseDown = function()
			...
			if(get(cs10)==0)then	set(cs10,1)
			elseif(get(cs10)==1)then set(cs10,0)
			end
...
function draw()
	...
	if(get(cs10)==0)then
		...
		drawText(fontMini,04,07,"FLAP",1,1,0,1)
		drawRectangle(2,30,28,28,1,1,0,1)
	elseif(get(cs10)==1)then 
			drawText(fontMini,04,17,"AUTO",0,1,0,1)
			drawText(fontMini,04,07,"FLAP",0,1,0,1)
